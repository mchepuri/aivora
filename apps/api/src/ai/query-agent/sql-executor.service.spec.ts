...
describe('SqlExecutorService', () => {
  ...
  describe('execute — SQL validation', () => {
    ...
    it('rejects a UNION-based cross-tenant bypass attempt', async () => {
      await expect(
        service.execute(
          `SELECT id FROM units_of_measure WHERE "tenantId" = '${TENANT_ID}' UNION SELECT passwordHash FROM users WHERE '1'='1'`,
          TENANT_ID,
        ),
      ).rejects.toThrow('forbidden SQL keyword');
    });
    ...
    it('rejects INTERSECT', async () => {
      await expect(
        service.execute(
          `SELECT id FROM units_of_measure WHERE "tenantId" = '${TENANT_ID}' INTERSECT SELECT id FROM users`,
          TENANT_ID,
        ),
      ).rejects.toThrow('forbidden SQL keyword');
    });
